...
function AskMe() {
  ...
  const prompt = `${info} \n the above paragraph is about me. Now, think from my perspective and using the information, draw inferences and give a relevant answer(take a guess using info on the web if you need to but dont mention it in the answer) to the following question in not more than 2 sentences, in a first person pov and remember that your answer is being displayed in my portfolio: \n ${question} \n`
  async function generateAnswer() {
    setAnswer('Let me think...!')
    const response = await axios({
      url:`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${import.meta.env.VITE_REACT_APP_APIKEY}`,
      method:'POST',
